...
void read_config() {
	...
	// On AmigaDOS, testing a missing file through the generic RealFS loader can
	// fail before it reports FileNotFoundError. Probe the native PROGDIR: path
	// first and use Widelands' defaults on a fresh installation.
	FILE* amiga_config = std::fopen(config_file.c_str(), "rb");
	if (amiga_config == nullptr) {
		log_info("No AmigaOS configuration file at %s; using defaults.", config_file.c_str());
		g_options.create_section("global");
		return;
	}
	std::fclose(amiga_config);
#endif
