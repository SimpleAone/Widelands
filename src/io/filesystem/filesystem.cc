...
#ifdef __amigaos4__
	// AmigaDOS absolute paths start with a volume or assign name followed by
	// a colon (for example "Work:Games/Widelands"). They never need a Unix
	// root slash in front of the volume name.
...
std::string FileSystem::get_working_directory() {
	...
	std::string result_path(cwd);
#ifdef __amigaos4__
	if (result_path.size() > 1 && result_path.front() == '/') {
		const std::string::size_type colon = result_path.find(':');
		const std::string::size_type slash = result_path.find('/', 1);
		if (colon != std::string::npos && (slash == std::string::npos || colon < slash)) {
			result_path.erase(result_path.begin());
		} else {
			const std::string::size_type volume_end =
			   slash == std::string::npos ? result_path.size() : slash;
			result_path = result_path.substr(1, volume_end - 1) + ":" +
			              (slash == std::string::npos ? "" : result_path.substr(slash + 1));
		}
	}
#endif
	return result_path;
...
std::string FileSystem::canonicalize_name(const std::string& path) const {
	...
	const bool amiga_volume =
#ifdef __amigaos4__
	   !components.empty() && components.front().find(':') != std::string::npos;
#else
	   false;
#endif
	for (i = components.begin(); i != components.end(); ++i) {
		if ((!first_component || !amiga_volume) &&
		    (canonpath.empty() || canonpath.back() != ':')) {
			canonpath.push_back('/');
		}
		canonpath += *i;
...
FileSystem& FileSystem::create(const std::string& root) {
	...
	std::string native_root = root;
#ifdef __amigaos4__
	// newlib may spell an AmigaDOS volume as "/Work:". Passing that form to
	// stat() opens an Assign requester for a volume literally named "/Work".
	// Normalize it before the very first host filesystem operation.
	if (native_root.size() > 1 && native_root.front() == '/') {
		const std::string::size_type colon = native_root.find(':');
		const std::string::size_type next_slash = native_root.find('/', 1);
		if (colon != std::string::npos &&
		    (next_slash == std::string::npos || colon < next_slash)) {
			native_root.erase(native_root.begin());
		} else {
			const std::string::size_type volume_end =
			   next_slash == std::string::npos ? native_root.size() : next_slash;
			native_root = native_root.substr(1, volume_end - 1) + ":" +
			              (next_slash == std::string::npos ? "" : native_root.substr(next_slash + 1));
		}
	}
#endif

	if (stat(native_root.c_str(), &statinfo) == -1) {
		if (errno == EBADF || errno == ENOENT || errno == ENOTDIR ||
#ifdef ELOOP
		    errno == ELOOP ||  // MinGW does not support ELOOP (yet)
#endif
		    errno == ENAMETOOLONG) {
			throw FileNotFoundError("FileSystem::create", native_root);
		}
		if (errno == EACCES) {
			throw FileAccessDeniedError("FileSystem::create", native_root);
		}
	}

	if (S_ISDIR(statinfo.st_mode)) {
		return *new RealFSImpl(native_root);
	}
	if (S_ISREG(statinfo.st_mode)) {  // TODO(unknown): ensure root is a zipfile
		return *new ZipFilesystem(native_root);
	}

	throw FileTypeError(
	   "FileSystem::create", native_root, "cannot create virtual filesystem from file or directory");
}
