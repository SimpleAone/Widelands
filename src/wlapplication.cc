...
#ifdef __amigaos4__
	// PROGDIR: always follows the executable, irrespective of the shell's
	// current directory or the volume/assign from which it was launched.
	executabledir = "PROGDIR:";
#endif
#ifdef __APPLE__
...
public:
	explicit GameLogicThread(std::atomic_bool* should_die)
	   ...
	   , thread_([]() {
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA LOGIC THREAD CHECKPOINT: native thread entered");
		#endif
		try {
			UI::Panel::logic_thread();
			#ifdef WL_AMIGAOS4_VIRTIO_GL
			log_info("AMIGA LOGIC THREAD CHECKPOINT: logic_thread returned normally");
			#endif
		} catch (const std::exception& e) {
			log_err("AMIGA LOGIC THREAD ERROR: uncaught std::exception: %s", e.what());
		} catch (...) {
			log_err("AMIGA LOGIC THREAD ERROR: uncaught non-standard exception");
		}
	})
	#endif
	{
	#ifdef __amigaos4__
		// GCC's native std::thread backend terminates its child task through the
		// owning process' newlib exit(), and pthreads.library is deliberately not
		// part of this port. Keep menu diagnostics single-threaded for now.
		log_info("AMIGA LOGIC THREAD: disabled (single-threaded newlib diagnostic mode)");
	#endif
	}
...
WLApplication::WLApplication(int const argc, char const* const* const argv)
   :
#ifdef WL_AMIGAOS4_VIRTIO_GL
     // AmigaDOS has no Unix-style home directory. Keep all writable state
     // beside the executable, using the native PROGDIR: assign.
     homedir_("PROGDIR:UserData")
#elif defined _WIN32
...
#endif
{
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	// Probe the required transport before starting sound or allocating the
	// graphics subsystem. A failed constructor otherwise leaves the sound
	// backend alive while main() is trying to report the exception.
	if (!VirtioGl::initialize_driver()) {
		const std::string driver_error = VirtioGl::last_error();
		log_err("AmigaOS4 startup aborted cleanly: %s", driver_error.c_str());
		VirtioGl::shutdown_driver();
		SDL_Quit();
		throw wexception("AmigaOS4 VirtIO driver unavailable: %s", driver_error.c_str());
	}
#endif
...

	{
		...
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA STARTUP CHECKPOINT: before SDL_PumpEvents");
		#endif
		SDL_PumpEvents();
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA STARTUP CHECKPOINT: after SDL_PumpEvents");
		#endif
		...
		while (SDL_PollEvent(&ev) != 0) {
			#ifdef WL_AMIGAOS4_VIRTIO_GL
			log_info("AMIGA STARTUP EVENT: type=0x%04x", ev.type);
			#endif
		...
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA STARTUP CHECKPOINT: initial event loop complete (handled=%d ignored=%d)",
		         handled, ignored);
		#endif
	}

	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: before init_mouse_cursor");
	#endif
	init_mouse_cursor();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after init_mouse_cursor");
	#endif
	 ...
	 */
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: before TTF_Init");
	#endif
	if (TTF_Init() == -1) {
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after TTF_Init");
	log_info("AMIGA STARTUP CHECKPOINT: before create_fonthandler");
	#endif

	UI::g_fh = UI::create_fonthandler(
	   g_image_cache, i18n::get_locale());  // This will create the fontset, so loading it first.
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after create_fonthandler");
	#endif
	verb_log_info("Font handler created");

	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: before set_template_dir");
	#endif
	set_template_dir("");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after set_template_dir");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: before get_render_target");
	#endif
	RenderTarget* r = g_gr->get_render_target();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after get_render_target, before draw_splashscreen");
	#endif
	draw_splashscreen(*r, _("Loading…"), 1.0f);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after draw_splashscreen, before refresh");
	#endif
	g_gr->refresh();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: before temporary cleanup");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after temporary cleanup, before songsets");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after songsets, before addons");
	#endif
	...
	initialize_g_addons();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after addons");
	#endif
	...
	write_config();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: after write_config");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STARTUP CHECKPOINT: WLApplication created");
	#endif
}
...
void WLApplication::init_mouse_cursor() {
	...
	if (mouse_position_ == Vector2i::zero()) {
		...
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: before SDL_GetWindowPosition");
		#endif
		SDL_GetWindowPosition(sdl_window, &window_x, &window_y);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after SDL_GetWindowPosition (%d,%d)", window_x, window_y);
		log_info("AMIGA CURSOR CHECKPOINT: before SDL_GetGlobalMouseState");
		#endif
		SDL_GetGlobalMouseState(&mouse_global_x, &mouse_global_y);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after SDL_GetGlobalMouseState (%d,%d)",
		...
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: before first SDL_PumpEvents");
		#endif
		SDL_PumpEvents();
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after first SDL_PumpEvents, before first warp");
		#endif
		SDL_WarpMouseInWindow(sdl_window, mouse_position_.x - 1, mouse_position_.y - 1);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after first warp, before second SDL_PumpEvents");
		#endif
		SDL_PumpEvents();
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after second SDL_PumpEvents, before SDL_Delay");
		#endif
		SDL_Delay(2);  // 1 tick doesn't work
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after SDL_Delay, before second warp");
		#endif
		SDL_WarpMouseInWindow(sdl_window, mouse_position_.x, mouse_position_.y);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after second warp, before final SDL_PumpEvents");
		#endif
		SDL_PumpEvents();
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA CURSOR CHECKPOINT: after final SDL_PumpEvents");
		#endif
	}
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CURSOR CHECKPOINT: before MouseCursor::initialize");
	#endif
	g_mouse_cursor->initialize(true);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CURSOR CHECKPOINT: after MouseCursor::initialize");
	#endif

	// TODO(tothxa): Do this in g_mouse_cursor->initialize() or the constructor too?
	g_mouse_cursor->change_wait(true);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CURSOR CHECKPOINT: after change_wait");
	#endif
}
...
#ifdef WL_NO_NETWORK
	if (multiplayer) {
		log_err("Multiplayer game templates are disabled in this build.");
		return;
	}
...
void WLApplication::run() {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA RUN CHECKPOINT: run start, before GameLogicThread");
	#endif
	GameLogicThread game_logic_thread(&should_die_);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA RUN CHECKPOINT: after GameLogicThread, before MainMenu");
	#endif

	FsMenu::MainMenu menu(game_type_ != GameType::kNone);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA RUN CHECKPOINT: after MainMenu");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA RUN CHECKPOINT: after cursor finalization, before crash reports");
	#endif

	check_crash_reports(menu);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA RUN CHECKPOINT: after crash reports, before game type switch");
	#endif
	...
	default:
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA RUN CHECKPOINT: before main menu loop");
		#endif
		menu.main_loop();
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA RUN CHECKPOINT: after main menu loop");
		#endif
	}
...
void WLApplication::handle_window_event(SDL_Event& ev) {
	...
	case SDL_WINDOWEVENT_CLOSE:
		log_info("Window close requested; shutting down cleanly.");
		should_die_ = true;
		break;
	default:
		break;
...
bool WLApplication::init_settings() {
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: before read_config" << std::endl;
	#endif
	// Read in the configuration file
	read_config();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: after read_config" << std::endl;
	#endif

	// Then parse the commandline - overwrites conffile settings
	handle_commandline_parameters();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: after commandline and filesystems" << std::endl;
	#endif
	...
	// Keyboard shortcuts
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: before init_shortcuts" << std::endl;
	#endif
	init_shortcuts();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: after init_shortcuts" << std::endl;
	#endif
	...
	update_mousewheel_settings();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: after mouse settings" << std::endl;
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: init_settings: complete" << std::endl;
	#endif
...
void WLApplication::handle_commandline_parameters() {
	...
	if (OptionalParameter datadir_option = get_commandline_option_value("datadir");
		...
		if (!found_datadir) {
		#ifdef WL_AMIGAOS4_VIRTIO_GL
			// PROGDIR: is an AmigaDOS assign. A slash immediately after the
			// colon would produce the invalid Unix-style path PROGDIR:/data.
			datadir_ = "PROGDIR:data";
			// The generic version-file probe currently fails inside the
			// filesystem adapter on native AmigaDOS volume paths. The directory
			// is still opened and validated by init_filesystems() below.
			found_datadir = true;
		#else
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: commandline: before init_filesystems, datadir='"
	          << datadir_ << "'" << std::endl;
	#endif
	init_filesystems();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: commandline: after init_filesystems" << std::endl;
	#endif

	// Do this now to have translated command line help.
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: commandline: before init_language" << std::endl;
	#endif
	init_language();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	std::cout << "AMIGA CHECKPOINT: commandline: after init_language" << std::endl;
	#endif
