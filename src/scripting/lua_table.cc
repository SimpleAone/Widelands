...
bool LuaTable::has_key_noexcept(const std::string& key) const {
	lua_pushlightuserdata(L_, const_cast<LuaTable*>(this));
	lua_rawget(L_, LUA_REGISTRYINDEX);
	lua_pushstring(L_, key);
	lua_rawget(L_, -2);
	const bool exists = !lua_isnil(L_, -1);
	lua_pop(L_, 2);
	if (exists) {
...
bool LuaTable::has_key_noexcept(const int key) const {
	lua_pushlightuserdata(L_, const_cast<LuaTable*>(this));
	lua_rawget(L_, LUA_REGISTRYINDEX);
	...
	lua_pop(L_, 2);
	if (exists) {
		accessed_keys_.insert(as_string(key));
	}
	return exists;
}
