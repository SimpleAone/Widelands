...
namespace {
MainMenu& amiga_capsule_parent(MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: before Window base parent");
#endif
	return menu;
}

int16_t amiga_capsule_x(MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: before window x");
#endif
	const int16_t result = menu.calc_desired_window_x(UI::Window::WindowLayoutID::kFsMenuDefault);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: after window x value=%d", result);
#endif
	return result;
}

int16_t amiga_capsule_y(MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: before window y");
#endif
	const int16_t result = menu.calc_desired_window_y(UI::Window::WindowLayoutID::kFsMenuDefault);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: after window y value=%d", result);
#endif
	return result;
}

int16_t amiga_capsule_width(MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: before window width");
#endif
	const int16_t result = menu.calc_desired_window_width(UI::Window::WindowLayoutID::kFsMenuDefault);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: after window width value=%d", result);
#endif
	return result;
}

int16_t amiga_capsule_height(MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: before window height");
#endif
	const int16_t result = menu.calc_desired_window_height(UI::Window::WindowLayoutID::kFsMenuDefault);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: after window height value=%d", result);
#endif
	return result;
}

UI::Panel* amiga_capsule_panel(UI::Panel* panel) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: Window base complete, before box");
#endif
	return panel;
}
}  // namespace
...
MenuCapsule::MenuCapsule(MainMenu& fsmm)
   : UI::Window(&amiga_capsule_parent(fsmm),
                UI::WindowStyle::kFsMenu,
                "menu",
                amiga_capsule_x(fsmm),
                amiga_capsule_y(fsmm),
                amiga_capsule_width(fsmm),
                amiga_capsule_height(fsmm),
                ""),
     box_(amiga_capsule_panel(this),
          UI::PanelStyle::kFsMenu,
          "main_box",
          0,
          0,
     ...
     fsmm_(fsmm) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: constructor body entered");
#endif
	set_visible(false);
	do_not_layout_on_resolution_change();
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA CAPSULE CHECKPOINT: constructor complete");
#endif
}
