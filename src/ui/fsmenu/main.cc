...
#ifndef WL_NO_NETWORK
#include "ui/fsmenu/internet_lobby.h"
#include "ui/fsmenu/login_box.h"
#include "ui/fsmenu/netsetup_lan.h"
#endif
...
template <typename T> T amiga_menu_checkpoint(const char* message, T value) {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA MAINMENU CHECKPOINT: %s", message);
	#else
	(void)message;
	#endif
	return value;
...
MainMenu& amiga_menu_checkpoint_menu(const char* message, MainMenu& menu) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA MAINMENU CHECKPOINT: %s", message);
#else
	(void)message;
...
MainMenu::MainMenu(const bool skip_init)
   : UI::Panel(nullptr,
               ...
               g_gr->get_xres(),
               g_gr->get_yres()),
     box_rect_(amiga_menu_checkpoint("after base Panel", 0), 0, 0, 0),
     butw_(get_w() * 7 / 20),
     buth_(get_h() * 9 / 200),
     padding_(buth_ / 3),
     vbox1_(this, UI::PanelStyle::kFsMenu, "vbox_1",
            amiga_menu_checkpoint("before vbox1 (scalar members complete)", 0), 0,
            UI::Box::Vertical, 0, 0, padding_),
     vbox2_(this, UI::PanelStyle::kFsMenu, "vbox_2",
            amiga_menu_checkpoint("after vbox1, before vbox2", 0), 0,
            UI::Box::Vertical, 0, 0, padding_),
     singleplayer_(&vbox1_,
                   "singleplayer",
                   amiga_menu_checkpoint("after vbox2, before singleplayer", 0),
                   0,
                   butw_,
                   6,
                   buth_,
     ...
     multiplayer_(&vbox1_,
                  "multiplayer",
                  amiga_menu_checkpoint("after singleplayer, before multiplayer", 0),
                  0,
                  butw_,
                  6,
                  buth_,
     ...
     replay_(&vbox1_,
             "replay",
             amiga_menu_checkpoint("after multiplayer, before replay", 0),
             amiga_menu_checkpoint("after replay, before editor", 0),
             butw_,
             6,
             buth_,
             "",
     ...
     editor_(&vbox1_,
             ...
             UI::ButtonStyle::kFsMenuMenu,
             [this](MenuTarget t) { action(t); }),
     addons_(&vbox2_, "addons", amiga_menu_checkpoint("after editor, before addons", 0), 0,
             butw_, buth_, UI::ButtonStyle::kFsMenuMenu, ""),
     options_(&vbox2_, "options", amiga_menu_checkpoint("after addons, before options", 0), 0,
              butw_, buth_, UI::ButtonStyle::kFsMenuMenu, ""),
     about_(&vbox2_, "about", amiga_menu_checkpoint("after options, before about", 0), 0,
            butw_, buth_, UI::ButtonStyle::kFsMenuMenu, ""),
     exit_(&vbox2_, "exit", amiga_menu_checkpoint("after about, before exit", 0), 0,
           butw_, buth_, UI::ButtonStyle::kFsMenuMenu, ""),
     clock_(this,
            UI::PanelStyle::kFsMenu,
            "clock",
            UI::FontStyle::kFsMenuLabel,
            amiga_menu_checkpoint("after exit, before clock", 0),
            0,
            0,
            0,
            "",
     ...
     version_(this,
              ...
              amiga_menu_checkpoint("after clock, before version", 0),
              0,
              0,
              0,
              "",
     ...
     copyright_(this,
                ...
                "copyright",
                UI::FontStyle::kFsMenuInfoPanelParagraph,
                amiga_menu_checkpoint("after version, before copyright", 0),
                0,
                0,
                0,
                "",
                UI::Align::kCenter),
     init_time_(amiga_menu_checkpoint("after copyright, before final scalar members", kNoSplash)),

     menu_capsule_(amiga_menu_checkpoint_menu("before MenuCapsule construction", *this)) {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA MAINMENU CHECKPOINT: constructor body entered");
	#endif
