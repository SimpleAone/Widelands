...
namespace {
template <typename T> T amiga_dropdown_checkpoint(const char* message, T value) {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: %s", message);
	#else
	(void)message;
	#endif
	return value;
...
int base_height(int button_dimension, UI::PanelStyle style) {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: base_height start");
	#endif
	int result =
	   std::max(button_dimension, text_height(g_style_manager->table_style(style).enabled()) + 2);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: base_height complete (%d)", result);
	#endif
	return result;
}
}  // namespace
...
BaseDropdown::BaseDropdown(UI::Panel* parent,
   ...
   : UI::Panel(parent,
     ...
     id_(amiga_dropdown_checkpoint("after base Panel", next_id_++)),
     max_list_items_(max_list_items),

     base_height_(amiga_dropdown_checkpoint("before cached base_height", base_height(button_dimension, style))),
     mouse_tolerance_(2 * default_button_size_small()),

     button_box_(this, style, "dropdown_button_box",
                 amiga_dropdown_checkpoint("before button_box", 0), 0,
                 UI::Box::Horizontal, w, get_h()),
     push_button_(amiga_dropdown_checkpoint("after button_box, before push_button",
     ...
     display_button_(&button_box_,
                     "dropdown_label",
                     amiga_dropdown_checkpoint("after push_button, before display_button", 0),
                     0,
...
{
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: constructor body entered");
	#endif
	if (label.empty()) {
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: before Listselect");
	#endif
	list_ = new UI::Listselect<uintptr_t>(
	   list_parent, "list", 0, 0, w, 0, style, ListselectLayout::kDropdown);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: after Listselect");
	#endif
	...
	set_value();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: after set_value");
	#endif
	...
	layout();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: after layout");
	#endif

	list_->initialization_complete();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA DROPDOWN CHECKPOINT: constructor complete");
	#endif
}
