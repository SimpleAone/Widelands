...
namespace {
bool amiga_trace_button(const std::string& name) {
...
Panel* amiga_button_parent(Panel* parent, const std::string& name) {
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: before Panel base constructor");
	}
	return parent;
}

ButtonStyle amiga_button_style(ButtonStyle style, const std::string& name) {
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: Panel base complete, before button style member");
	}
	return style;
}
}  // namespace
...
Button::Button  //  Common constructor
   (Panel* const parent,
    ...
    ImageMode mode)
   : Panel(amiga_button_parent(parent, name), to_panel_style(init_style), name, x, y, w, h, tooltip_text),
     highlighted_(false),
     pressed_(false),
     enabled_(true),
     ...
     button_style_(amiga_button_style(init_style, name)) {
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: common constructor body entered");
	}
	set_thinks(false);
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: after set_thinks");
	}
	set_snap_target(true);
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: after set_snap_target");
	}
	set_can_focus(enabled_);
	if (amiga_trace_button(name)) {
		log_info("AMIGA BUTTON CHECKPOINT: common constructor complete");
	}
}
...
void Button::draw(RenderTarget& dst) {
	...
	if (title_image_ != nullptr) {
		...
		std::shared_ptr<const UI::RenderedText> rendered_text =
		   autofit_text(richtext_escape(title_), style_to_use.font(),
		                get_inner_w() - 2 * default_button_image_margin());
