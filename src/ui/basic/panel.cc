...
void Panel::logic_thread() {
	...
	log_info("AMIGA LOGIC THREAD CHECKPOINT: before set_logic_thread");
	#endif
	set_logic_thread();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA LOGIC THREAD CHECKPOINT: after set_logic_thread");
	#endif
	logic_thread_running_ = true;
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA LOGIC THREAD CHECKPOINT: before WLApplication::get");
	#endif
	const WLApplication& app = WLApplication::get();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA LOGIC THREAD CHECKPOINT: after WLApplication::get, entering loop");
	#endif
