...
namespace {
bool amiga_trace_window(const std::string& name) {
...
uint32_t amiga_window_outer_width(WindowStyle style, uint32_t width, const std::string& name) {
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: before width border style lookup");
	}
	const WindowStyleInfo& info = g_style_manager->window_style(style);
	const uint32_t result = width + info.left_border_thickness() + info.right_border_thickness();
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: width border lookup complete outer=%u", result);
	}
	return result;
}

uint32_t amiga_window_outer_height(WindowStyle style, uint32_t height, const std::string& name) {
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: before height border style lookup");
	}
	const WindowStyleInfo& info = g_style_manager->window_style(style);
	const uint32_t result = height + info.top_border_thickness() + info.bottom_border_thickness();
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: height border lookup complete outer=%u", result);
	}
	return result;
}

uint32_t amiga_window_old_height(WindowStyle style, uint32_t height, const std::string& name) {
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: Panel base complete, before old height lookup");
	}
	const WindowStyleInfo& info = g_style_manager->window_style(style);
	const uint32_t result = height + info.top_border_thickness() + info.bottom_border_thickness();
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: old height lookup complete value=%u", result);
	}
	return result;
}

Button* amiga_window_close_button(Window* window, WindowStyle style, const std::string& name) {
#if defined(WL_AMIGAOS4_VIRTIO_NO_SHADERS)
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: no-shader mode skips close button image");
	}
	const uint32_t button_size = g_style_manager->window_style(style).button_size();
	return new Button(
	   window, "b_close", 0, 0, button_size, button_size,
	   style == WindowStyle::kWui ? ButtonStyle::kWuiSecondary : ButtonStyle::kFsMenuSecondary,
	   "", _("Close"));
#else
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: before close button style lookup");
	}
	const WindowStyleInfo& info = g_style_manager->window_style(style);
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: close button style ready, before image lookup '%s'",
		         info.button_close().c_str());
	}
	const Image* image = g_image_cache->get(info.button_close());
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: close image ready, before Button constructor");
	}
	Button* button = new Button(
	   window, "b_close", 0, 0, info.button_size(), info.button_size(),
	   style == WindowStyle::kWui ? ButtonStyle::kWuiSecondary : ButtonStyle::kFsMenuSecondary,
	   image, _("Close"));
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: close Button constructor complete");
	}
	return button;
#endif
}

Button* amiga_window_empty_button(Window* window,
                                  WindowStyle style,
                                  const std::string& name,
                                  const char* button_name) {
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: before %s Button constructor", button_name);
	}
	const uint32_t button_size = g_style_manager->window_style(style).button_size();
	Button* button = new Button(
	   window, button_name, 0, 0, button_size, button_size,
	   style == WindowStyle::kWui ? ButtonStyle::kWuiSecondary : ButtonStyle::kFsMenuSecondary,
	   "", "");
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: %s Button constructor complete", button_name);
	}
	return button;
}
}  // namespace
...
Window::Window(Panel* const parent,
   ...
   : Panel(parent,
           ...
           x,
           y,
           amiga_window_outer_width(s, w, name),
           amiga_window_outer_height(s, h, name)),
     window_style_(s),

     oldh_(amiga_window_old_height(s, h, name)),

     button_close_(amiga_window_close_button(this, s, name)),
	 button_pin_(amiga_window_empty_button(this, s, name, "b_pin")),
	 button_minimize_(amiga_window_empty_button(this, s, name, "b_minimize")) {
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: constructor body entered");
	}
	set_title(title);
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: after set_title");
	}
	...
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: callbacks connected, before toolbar update");
	}
	update_toolbar_buttons();
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: after toolbar update, before border");
	}
	...
	focus();
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: before resolution subscriber");
	}

	graphic_resolution_changed_subscriber_ = Notifications::subscribe<GraphicResolutionChanged>(
	   [this](const GraphicResolutionChanged& note) { on_resolution_changed_note(note); });
	if (amiga_trace_window(name)) {
		log_info("AMIGA WINDOW CHECKPOINT: constructor complete");
	}
}
...
#if defined(WL_AMIGAOS4_VIRTIO_NO_SHADERS)
	// The no-shader diagnostic configuration has no GL-backed toolbar images.
	// Keep the button objects and their state so Window layout and event handling
	// remain valid, but avoid loading decorative textures.
...
void Window::layout() {
	...
	button_close_->set_pos(Vector2i(
	   get_inner_w() + window_style_info().button_close_x(), window_style_info().buttons_y()));
	button_pin_->set_pos(
