...
std::string TextStream::till_any_or_end(const std::string& chars) {
	// On AmigaOS4 a simple final word must not enter the exception path below.
	// Besides being needlessly expensive for ordinary richtext, the old path
	// consumed one byte past end_ after catching EndOfTextImpl.
#ifdef WL_AMIGAOS4_VIRTIO_GL
	for (size_t j = pos_; j < end_; ++j) {
		for (const char ch : chars) {
			if (ch == text_[j]) {
				return till_any(chars);
			}
		}
	}
	std::string rv = text_.substr(pos_, end_ - pos_);
	consume(end_ - pos_);
	return rv;
#else
	std::string rv;
