...
FontSet::FontSet(const std::string& fontset_name) {
	...
	log_info("AMIGA FONT CHECKPOINT: FontSet '%s' before parse", fontset_name.c_str());
	#endif
	parse_fontset(fontset_name);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: FontSet '%s' after parse", fontset_name.c_str());
	#endif
	assert(!serif_.empty());
...
FontSet::FontSet(const std::string& fontset_name, LuaTable& fonts_table) {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: FontSet '%s' before shared-table parse",
	         fontset_name.c_str());
	#endif
	parse_fontset(fontset_name, fonts_table);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: FontSet '%s' after shared-table parse",
	         fontset_name.c_str());
	#endif
	assert(!serif_.empty());
	assert(!serif_bold_.empty());
...
void FontSet::parse_fontset(const std::string& fontset_name, LuaTable& fonts_table) {
	std::string direction_string;
	size_offset_ = 0;

	// Initialize with default fontset
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' before get default table", fontset_name.c_str());
	#endif
	std::unique_ptr<LuaTable> default_font_table = fonts_table.get_table("default");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' after get default table, before set_fonts",
	         fontset_name.c_str());
	#endif
	set_fonts(*default_font_table, kFallbackFont);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' after default set_fonts, before direction",
	         fontset_name.c_str());
	#endif
	direction_string = default_font_table->get_string("direction");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' after direction, before representative character",
	         fontset_name.c_str());
	#endif
	representative_character_ = default_font_table->get_string("representative_character");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' after representative character, before size offset",
	         fontset_name.c_str());
	#endif
	size_offset_ = default_font_table->get_int("size_offset");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT PARSE: '%s' after default fields, before has_key",
	         fontset_name.c_str());
	#endif
	...
	try {
		...
		name_ = fontset_name;
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT PARSE: '%s' after has_key, before get selected table",
		         fontset_name.c_str());
		#endif
		std::unique_ptr<LuaTable> font_set_table = fonts_table.get_table(fontset_name);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT PARSE: '%s' after get selected table, before selected representative",
		         fontset_name.c_str());
		#endif
		representative_character_ = font_set_table->get_string("representative_character");
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT PARSE: '%s' after selected representative, before selected set_fonts",
		         fontset_name.c_str());
		#endif

		set_fonts(*font_set_table, serif_);
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT PARSE: '%s' after selected set_fonts", fontset_name.c_str());
		#endif
...
FontSets::FontSets() {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: FontSets constructor start");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: before LuaInterface for fontsets");
	#endif
	LuaInterface lua;
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: after LuaInterface, before fonts.lua");
	#endif

	// Read all fontsets
	std::unique_ptr<LuaTable> fontsets_table(lua.run_script("i18n/fonts.lua"));
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: after fonts.lua, before fontset keys");
	#endif
	for (const std::string& fontset_name : fontsets_table->keys<std::string>()) {
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT CHECKPOINT: before creating FontSet '%s'", fontset_name.c_str());
		#endif
		assert(fontset_selectors.count(fontset_name) == 1);
		FontSets::Selector selector = fontset_selectors.at(fontset_name);
		fontsets[selector] =
		   std::unique_ptr<FontSet>(new FontSet(fontset_name, *fontsets_table));
		#ifdef WL_AMIGAOS4_VIRTIO_GL
		log_info("AMIGA FONT CHECKPOINT: after creating FontSet '%s'", fontset_name.c_str());
		#endif
	}
	fontsets_table
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: before listing locale directory");
	#endif
	FilenameSet files = g_fs->list_directory("locale");
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA FONT CHECKPOINT: after listing locale directory (%u files)",
