...
void set_icon(SDL_Window* sdl_window) {
	...
	log_info("AMIGA ICON CHECKPOINT: before image load: %s", icon_name.c_str());
	#endif
	SDL_Surface* s = load_image_as_sdl_surface(icon_name, g_fs);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA ICON CHECKPOINT: image loaded %dx%d format=%s", s->w, s->h,
	         SDL_GetPixelFormatName(s->format->format));
	log_info("AMIGA ICON CHECKPOINT: before SDL_SetWindowIcon");
	#endif
	SDL_SetWindowIcon(sdl_window, s);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA ICON CHECKPOINT: after SDL_SetWindowIcon");
	#endif
	SDL_FreeSurface(s);
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA ICON CHECKPOINT: after SDL_FreeSurface");
	#endif
}
...
void Graphic::initialize(const TraceGl& trace_gl,
	...
	if (!VirtioGl::initialize_driver()) {
		throw wexception("AmigaOS4 VirtIO GL initialization failed: %s", VirtioGl::last_error());
	}
#endif

#ifndef WL_AMIGAOS4_VIRTIO_GL
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	VirtioGl::set_output_window(sdl_window_);
#endif
	SDL_SetWindowMinimumSize(sdl_window_, kMinimumResolutionW, kMinimumResolutionH);

	GLint max;
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("Graphics: AmigaOS4 direct VirtIO window; no SDL GL context\n");
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: before set_fullscreen");
#endif
	set_fullscreen(init_fullscreen);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: after set_fullscreen");
#endif
	if (init_maximized) {
		set_maximized(true);
	}
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: before resolution_changed");
#endif
	resolution_changed();
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: after resolution_changed");
#endif

#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: before window title");
#endif
	SDL_SetWindowTitle(sdl_window_, ("Widelands " + build_ver_details()).c_str());
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: after window title, before icon");
#endif
	set_icon(sdl_window_);
#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA GRAPHICS CHECKPOINT: after icon, before initial present");
#endif

#ifdef WL_AMIGAOS4_VIRTIO_GL
	if (!VirtioGl::present()) {
		throw wexception("AmigaOS4 VirtIO initial present failed: %s", VirtioGl::last_error());
	}
	log_info("AMIGA GRAPHICS CHECKPOINT: after initial present");
#else
...
Graphic::~Graphic() {
	...
	if (sdl_window_ != nullptr) {
#ifdef WL_AMIGAOS4_VIRTIO_GL
		VirtioGl::set_output_window(nullptr);
#endif
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	if (VirtioGl::driver_ready() && !VirtioGl::create_frame_target(new_w, new_h)) {
		throw wexception("AmigaOS4 VirtIO frame-target resize failed: %s",
		                 VirtioGl::last_error());
	}
#endif
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	if (!VirtioGl::present()) {
		throw wexception("AmigaOS4 VirtIO present failed: %s", VirtioGl::last_error());
	}
#else
	SDL_GL_SwapWindow(sdl_window_);
#endif
