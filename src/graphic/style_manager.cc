...
void set_template_dir(std::string dir) {
	...
	log_info("AMIGA STYLE CHECKPOINT: set_template_dir start");
	#endif
	if (dir.empty()) {
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before template directory check '%s'", dir.c_str());
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before do_update_g_style_manager");
	#endif
	do_update_g_style_manager();
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: after do_update_g_style_manager, before dither mask");
	#endif

	RenderQueue::instance().set_dither_mask(resolve_template_image_filename("world/pics/edge.png"));
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: after dither mask");
	#endif
}
...
UI::PanelStyleInfo* read_panel_style(const LuaTable& table) {
	...
	const Image* panel_image = nullptr;
	#if defined(WL_AMIGAOS4_VIRTIO_NO_SHADERS)
	if (!image.empty()) {
		log_info("AMIGA STYLE: no-shader mode skips panel GL image '%s'", image.c_str());
	}
	#else
	panel_image = image.empty() ? nullptr : g_image_cache->get(image);
	#endif
...
StyleManager::StyleManager() {
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: StyleManager constructor start");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before style LuaInterface");
	#endif
	LuaInterface lua;
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: after style LuaInterface, before init.lua");
	#endif
	std::unique_ptr<LuaTable> table(lua.run_script(format("%1%init.lua", template_dir())));
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: after style init.lua, before buttons");
	#endif
	...
	// Tabpanels
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before tabpanels");
	#endif
	section = "tabpanels";
	element_table = try_section_or_empty(lua, *table, section);
	sub = "fsmenu";
	...
	// Editboxes
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before editboxes");
	#endif
	section = "editboxes";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Dropdowns
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before dropdowns");
	#endif
	section = "dropdowns";
	element_table = try_section_or_empty(lua, *table, section);
	sub = "fsmenu";
	...
	// Scrollbars
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before scrollbars");
	#endif
	section = "scrollbars";
	element_table = try_section_or_empty(lua, *table, section);
	sub = "fsmenu";
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before building statistics");
	#endif
	...
	// Progress bars
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before progress bars");
	#endif
	section = "progressbar";
	element_table = try_section_or_empty(lua, *table, section);
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before tables");
	#endif
	section = "tables";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Windows
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before windows");
	#endif
	section = "windows";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Statistics plot
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before statistics plot");
	#endif
	...
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before ware info");
	#endif
	section = "wareinfo";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Special elements
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before special elements");
	#endif
	std::string key;
	...
	// Fonts
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before fonts");
	#endif
	section = "fonts";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Paragraphs
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before paragraphs");
	#endif
	section = "paragraphs";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Colors
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before colors");
	#endif
	section = "colors";
	element_table = try_section_or_empty(lua, *table, section);
	...
	// Sizes
	#ifdef WL_AMIGAOS4_VIRTIO_GL
	log_info("AMIGA STYLE CHECKPOINT: before sizes");
	#endif
...
void StyleManager::add_ware_info_style(UI::WareInfoStyle style,
	...
	if (table.has_key(key)) {
		...
		const std::string icon_background_image = style_table->get_string("icon_background_image");
		const Image* icon_background = nullptr;
		#if defined(WL_AMIGAOS4_VIRTIO_NO_SHADERS)
		log_info("AMIGA STYLE: no-shader mode skips ware-info GL image '%s'",
		         icon_background_image.c_str());
		#else
		icon_background = g_image_cache->get(icon_background_image);
		#endif
...
void StyleManager::add_window_style(UI::WindowStyle style,
	...
	if (table.has_key(key)) {
		std::unique_ptr<LuaTable> style_table = table.get_table(key);
		auto load_window_image = [&style_table](const char* field) -> const Image* {
			const std::string filename = style_table->get_string(field);
			#if defined(WL_AMIGAOS4_VIRTIO_NO_SHADERS)
			log_info("AMIGA STYLE: no-shader mode skips window GL image '%s'", filename.c_str());
			return nullptr;
			#else
		...
		w_style = new UI::WindowStyleInfo(
		   ...
		   load_window_image("border_top"), load_window_image("border_bottom"),
		   load_window_image("border_right"), load_window_image("border_left"),
		   load_window_image("corner_bottom_left"), load_window_image("corner_bottom_right"),
		   load_window_image("corner_top_left"), load_window_image("corner_top_right"),
		   load_window_image("corner_minimal_left"), load_window_image("corner_minimal_right"),
		   load_window_image("background"),
