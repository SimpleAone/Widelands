...
SDL_GLContext initialize(
   ...
   GLint* max_texture_size) {
	...
	(void)sdl_window;
	/* The direct VirtIO frontend owns the VirGL context.  There deliberately is
	 * no SDL OpenGL context on this backend. */
...
#ifdef WL_AMIGAOS4_VIRTIO_GL
	// The compiled-in frontend exports the GL entry points directly. Context
	// state will move here once the first VirtIO command encoder is connected.
...
#else
	verb_log_info("Graphics: SDL GL attributes are not applicable to direct VirtIO\n");
#endif
